function [answer_code,rtime,response_code,tasktype]= RunExperiment(w,wRect,tiralnum,frame_duration,maker,...
    ...
    subID,MovieFrames,a,b,resttrial,experimenttype,image,MovieData)
% practice
%@tiralnum = 24
%@resttrial = 12
%@experimenttype = 1
        ...
        for i=1:MovieFrames
            for np=1:NumMovie
                ...
                Screen('DrawTexture',w,act{InputNameIndex(np)}{i},[],...
                    [position_presentation(InputNameIndex(np),1)-CoodinateScale*MovieData{InputNameIndex(np)}((12*(i-1)+j),2)-square_width...
                    position_presentation(InputNameIndex(np),2)-CoodinateScale*MovieData{InputNameIndex(np)}((12*(i-1)+j),3)-square_width...
                    position_presentation(InputNameIndex(np),1)-CoodinateScale*MovieData{InputNameIndex(np)}((12*(i-1)+j),2)-square_width...
                    position_presentation(InputNameIndex(np),2)--CoodinateScale*MovieData{InputNameIndex(np)}((12*(i-1)+j),3)-square_width]);
                end
            end
